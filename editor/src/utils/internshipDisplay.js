...
function jaDisplay(value) {
  ...
  return String(value)
    ...
    .replace(/^Occasional hiring; no fixed date stated$/i, '随時採用（締切日の記載なし）')
    .replace(/^Flexible - Any SpaceX Site$/i, '勤務地柔軟（SpaceX各拠点）')
    .replace(/Remote in USA\s*\|\s*Remote in Canada/gi, '米国・カナダ国内リモート')
    .replace(/Remote in USA/gi, '米国内リモート')
    .replace(/Remote in Canada/gi, 'カナダ国内リモート')
