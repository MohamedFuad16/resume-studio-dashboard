...
test.describe('Internship Portal — app shell', () => {
  ...
  test('renders the shell with language switcher and primary navigation', async ({ page }) => {
    ...
    await expect(page.locator('.top-nav-btn', { hasText: 'Dashboard' })).toBeVisible();
    await expect(page.locator('.top-nav-btn', { hasText: 'Internship Radar' })).toBeVisible();
    await expect(page.locator('.top-nav-btn', { hasText: 'Editor' })).toBeVisible();
  });
